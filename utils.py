...
class DGTNet_parameter():
    """
    The DGTNet's parameter is defined here. It contains model parameter and training parameter. Different dataset use different parameter setting.
    ...
    def __init__(self, dataset_name:str):
        ...
        # PEMS07 12672*228
            ...
            self.model_parameters = {
            'l_backcast': 12,                       # 时间窗口
            'd_edge': 5,                            # 时间序列分解为5个部分
            'd_model': 256,                         # model hidden layer
            'N': 8,                                 # number of Self_Attention_Block 重复8次
            'h': 16,                                # Multi-attention heads
            'N_dense': 1,                           # Sequential feed forward layers
            'n_output': 12*228,                          # 预测的值是336，原来的数据集是有7个特征，如果只预测一个值，则只有48*1
            'n_nodes':228,
            'lambda_attention': 0.33,               # 各个IMF的权重
            'lambda_imf_1': 0.0999,
            'lambda_imf_2': 0.0801,
            'lambda_imf_3': 0.06,
            'lambda_imf_4': 0.0399,
        ...
        # Pems03 26207 * 358
            ...
            self.model_parameters = {
                ...
                'h': 16,                        # Multi-attention heads
                'N_dense': 1,                   # Sequential feed forward layers
                'n_output': 12*358,                  # 预测的值是336，原来的数据集是有7个特征，如果只预测一个值，则只有48*1
                'n_nodes': 358,
                'lambda_attention': 0.33,  # 各个IMF的权重
                'lambda_imf_1': 0.0999,
                'lambda_imf_2': 0.0801,
        ...
        # PEMS_BAY 52115*325
            ...
            self.model_parameters = {
                ...
                'h': 16,                            # Multi-attention heads
                'N_dense': 1,                       # Sequential feed forward layers
                'n_output': 12*325,                      # 预测的值是336，原来的数据集是有7个特征，如果只预测一个值，则只有48*1
                'n_nodes': 325,
                'lambda_attention': 0.33,           # 各个IMF的权重
                'lambda_imf_1': 0.0999,
                'lambda_imf_2': 0.0801,
            ...
            self.model_parameters = {
                ...
                'h': 16,                            # Multi-attention heads
                'N_dense': 1,                       # Sequential feed forward layers
                'n_output': 12*206,                  # 预测的值是336，原来的数据集是有7个特征，如果只预测一个值，则只有48*1
                'n_nodes': 206,
                'lambda_attention': 0.33,           # 各个IMF的权重
                'lambda_imf_1': 0.0999,
                'lambda_imf_2': 0.0801,
...
def calculate_loss(y_true, y_pred, loss_function_name, criterion, device):
    ...
    if loss_function_name == 'mae':
        ...
        y_true = y_true.view(-1, y_pred.size(-1)).to(device)  # 调整y_true的形状与y_pred相匹配
        loss = criterion(y_pred, y_true)

    elif loss_function_name == 'smoothed mae':
        # y_true, y_pred = y_true.flatten(), y_pred.flatten()
        y_true = y_true.view(-1, y_pred.size(-1)).to(device)
        loss = criterion(y_pred, y_true)
