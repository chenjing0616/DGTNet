...
# Create the DGTNet Model
def make_DGTNet_model(l_backcast, n_nodes, d_edge=5, N=2, d_model=128, h=8, dropout=0.1,
    ...
    """
    Construct a DGTNet model from input hypterparameters.
    ...
    model = Time_Series_Attention_Transformer(
        ...
        Time_Series_Self_Attention(          # 2
            Self_Attention_Block(            # 包括两个部分 (TSMHA)time series mutli-head self-attention+ (SFFN)feed forward
                d_model, copy.deepcopy(TSMHA), copy.deepcopy(SFFN), dropout, scale_norm, use_adapter
                ),
            N,           # 重复n次
            scale_norm
            ),
        Graph_Representation_Readout(        # 3
...
# =======================================================================================
### Main algorithm of DGTNet
class Time_Series_Attention_Transformer(nn.Module):
    """
    Main part of DGTNet model, it consists of time_embeddings, time_series_self_attention, and graph_representation_readout.
    """
